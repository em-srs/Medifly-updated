...
5. **Docker** (Optional; for containerized backend deployment and cloud hosting using the production `Dockerfile` in the backend directory. The Dockerfile executes `gradlew` using an explicit `sh` shell wrapper to support seamless Linux-based builds on platforms like Render).
