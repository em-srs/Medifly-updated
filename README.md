...
5. **Docker** (Optional; for containerized backend deployment and cloud hosting using the production `Dockerfile` in the backend directory. The Dockerfile utilizes wildcard copying for build artifacts and relies on an explicit `app.jar` archive filename configured in Gradle, executing `gradlew` via an explicit `sh` shell wrapper to support seamless Linux-based builds on platforms like Render).
